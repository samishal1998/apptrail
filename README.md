...
In v0.4+, Traefik host aliases on the same router/path form one application with alternate launch addresses. The owner can expand those addresses or select a primary URL in the app editor; public pages only expose the primary URL. Generated internal routers (ACME, API, dashboard, ping, and redirects) are excluded. On rescan, older alias records consolidate while preserving preferences and placements; conflicting overrides/explicit IDs block the merge instead of losing data. Uncustomized internal noise is cleaned up automatically, while curated entries are retained.
